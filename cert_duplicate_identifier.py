...
# Output
OUTPUT_DIRECTORY = "projectLogs"
JSONLINE_EXTENSION = '.jsonline'

# Constants
TAG_NEW = "NW"  # Identifies the fingerprint when it is encountered for the first time
TAG_EXISTING = "EX"  # Tag the fingerprint as already seen
...
class MapValue(object):
    ...
    def __init__(self, start, length, tag, path, end_offset):
        ...
        :param path: Filename of a fingerprint
        :param end_offset: End offset of a fingerprint in the destination file
        '''
...
class CertificateDuplicateIdentifier(object):
    ...
    def append(self, finger_print, map_val, current_obj):
        ...
        if map_val.tag == TAG_NEW:
            with open(self.input_file, mode='r') as temp_input_file_fp:
                ...
                # Create text in the desired output format
                temp_text = '{"fingerprint": "%s", "certificates": [' % finger_print

                # Creates an output file for a fingerprint when a duplicate has been identified for it for the first time
            ...
            # Append a new duplicate json object for an existing fingerprint output file
    ...
    def process_data(self, input_file, output_directory="", output_file=""):
        ...
        with open(self.input_file, mode='rb') as input_file_fp:
            ...
            while line:
                ...
                # Retrive fingerprint from the json object using ijson (avoids using json loads for faster execution)
        ...
        print_to_console("Data processed in: {0}".format(time.time() - start_time))
    ...
    def merge_and_create_output(self, clear_temp=True):
        ...
        print_to_console("Data cleaned in: {0}".format(time.time() - start_time))
